  ...
  <ion-page>
    ...
    <ion-header>
      <ion-toolbar>
        ...
        <ion-title>Approved Timesheets</ion-title>
      </ion-toolbar>
    </ion-header>
...
onIonViewWillEnter(async () => {
  ...
  await loadApprovedTimesheets();
  initialLoading.value = false; // hide global spinner
});

/* ================= API CALLS ================= */
const loadApprovedTimesheets = async () => {
  try {
    loading.value = true;
    ...
    console.error("Failed to load approved timesheets", err);
  } finally {
    loading.value = false;
  }
};
...
const confirmEmail = async (displayDate: string, timesheetId: number) => {
  if (!userEmail.value) {
  ...
  const msg =
    `Weekend: ${displayDate}\n\n` +
    `Send the approved timesheet to:\n${userEmail.value}`;

  const alert = await alertController.create({
    header: "Email Timesheet",
    message: msg,
      ...
      { text: "Cancel", role: "cancel" },
      {
        ...
        handler: async () => {
          try {
            ...
            await showMessage(
              "Email Sent",
              "A copy of your approved timesheet has been sent."
            );
